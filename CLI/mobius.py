# Möbius Transforms CLI - 2022-09-01

# Python code based upon Henry Segerman's "Mobius Transformations (2016)" project:
...
def inf_zero_one_to_triple(p,q,r):
    ...
    M = np.vstack((p,q)).T
    mu,lam = np.linalg.lstsq(M,r,rcond=-1)[0]
...
def two_triples_to_SL(a1,b1,c1,a2,b2,c2):
    ...
    return np.linalg.lstsq(M1.T, M2.T, rcond=-1)[0].T
...
def zoom_in_on_pixel_coords(p, zoom_factor, size):
    ...
    return np.dot(np.linalg.lstsq(M_rot, M_scl, rcond=-1)[0], M_rot)
...
def apply_SL2C_elt_to_image(M_SL2C, src_image, out_size=None):
    ...
    Minv = np.linalg.lstsq(M_SL2C, np.eye(2), rcond=-1)[0]
...
def main():
    ...
    source_image = np.array(Image.open(in_path),dtype=np.float32)
    sz = source_image.shape[:-1]

    M = zoom_in_on_pixel_coords((x_degree, y_degree), zoom, sz)
